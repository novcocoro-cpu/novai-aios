...
export async function POST(req: NextRequest) {
  try {
    ...
    let systemPrompt = DEFAULT_PROMPTS.prompt_sales;
    let temperature = 0.3;
    let model = DEFAULT_PROMPTS.model_sales;
    if (isSupabaseConfigured) {
      const { data, error } = await supabaseSchema("shared").from("settings").select("key, value").in("key", ["prompt_sales", "temperature", "model_sales"]);
      if (error) console.error("[sales] settings fetch failed:", error.message);
      if (data) {
        for (const row of data) {
          if (row.key === "prompt_sales") systemPrompt = row.value;
          if (row.key === "temperature") temperature = parseFloat(row.value);
          if (row.key === "model_sales") model = row.value;
        }
      }
    }
    ...
    const title = messages[0]?.content?.slice(0, 50) || "営業相談";
    const savedId = await saveConversation("sales", conversationId, allMessages, title);
